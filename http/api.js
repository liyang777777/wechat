  ...
  getCats(){
   ...
   getMinor(){
    ...
    relatedRecommendedBooks (book_id) { // @param book_id 书籍id
      return fly.get(`/book/book_id=${book_id}/recommend`)
    },
    // 作者名下的书籍
    ...
    bookSources (book_id) {  // @param book_id 书籍id
     ...
     bookChapters (id) {  // @param id 书源id
      return  fly.get(`/atoc/id=${id}?view=chapters`)
    },
     // 书籍章节 根据书id
     ...
     chapterContent (link) {  // @param link 章节link
      return fly.get(`https://chapter2.zhuishushenqi.com/chapter/link=${encodeURIComponent(link)}`)
    },
     //搜索热词
     hotWord(){
    ...
    rankInfo (rank_id) { //@param rank_id 分类ID
      return fly.get(`/ranking/rank_id=${rank_id}`)
    },
     // 讨论
    ...
    detail (id) {  // @param id 书单id
      return fly.get(`/book-list/id=${book_id}`)
    },
}
